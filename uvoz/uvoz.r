...
BDP <- read_csv("podatki/bdp.csv", na="-", col_select = c(7, 8, 9),
                locale=locale(encoding="Windows-1250")) %>% dplyr::rename(TIME = OBS_VALUE) %>% dplyr::rename(GEO = TIME_PERIOD)
tretja <- read_csv("podatki/tretja_stopnja.csv", na="-", col_select = c(1, 6, 7),
                   locale=locale(encoding="Windows-1250")) %>% dplyr::rename(okrajsava = LOCATION) %>% dplyr::rename(Year = TIME) %>% dplyr::rename(Education2 = Value)
skupaj <- left_join(D, BDP, by= c("TIME", "GEO")) %>% dplyr::rename(BDP = OBS_FLAG) %>% dplyr::mutate(BDP=parse_number(BDP))
...
tabela1 <- tabela1 %>% mutate(Education = ifelse(is.na(Education), Education2, Education)) %>% dplyr::select(-Education2, -okrajsava)


koliko_vsega <- read_csv("podatki/po_izobr_koliko_vsega.csv", na=":", col_select = c(-3, -5, -7),
                              locale=locale(encoding="Windows-1250")) %>% dplyr::rename(Year = TIME) %>% dplyr::rename(Country = GEO) %>% dplyr::rename(Stopnja = IND_TYPE) %>% dplyr::rename(Koliko_vseh_nakupov_je_opravila_ta_skupina = Value)
kaj_so <- read_csv("podatki/po_izobrazbi_kaj.csv", na=":", col_select = c(-5, -7),
                   locale=locale(encoding="Windows-1250")) %>% pivot_wider(names_from = "INDIC_IS", values_from = "Value") %>% dplyr::rename(Year = TIME) %>% dplyr::rename(Country = GEO) %>% dplyr::rename(Stopnja = IND_TYPE)
...
tabela3 <- read_csv("podatki/kaj_kupujejo1.csv", na=":", col_select=c(-4,-5,-7), 
                                   locale=locale(encoding="Windows-1250")) %>% pivot_wider(names_from = "INDIC_IS", values_from = "Value") %>% dplyr::rename(Year = TIME) %>% dplyr::rename(Country = GEO)
